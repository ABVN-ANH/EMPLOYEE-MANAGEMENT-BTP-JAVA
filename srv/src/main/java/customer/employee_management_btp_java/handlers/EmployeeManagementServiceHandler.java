package customer.employee_management_btp_java.handlers;
...
public class EmployeeManagementServiceHandler implements EventHandler {
    ...
    public EmployeeManagementServiceHandler(PersistenceService db) {
        this.db = db;
    }
    ...
    public void onCalculateEmployeeSalary(CalculateEmployeeSalaryContext context) {
        ...
        Optional<Employees> employeeOpt = db.run(Select.from(CDS_NAME).where(b -> b.get("ID").eq(employeeId))).first(Employees.class);
    ...
    private BigDecimal calculateSalary(String roleId, Date hireDate) {
        if (roleId == null || hireDate == null) return null;
    ...
    private Date toSqlDate(Object dateObj) {
        ...
        if (dateObj instanceof LocalDate) return Date.valueOf((LocalDate) dateObj);
        return null;
    }
}
